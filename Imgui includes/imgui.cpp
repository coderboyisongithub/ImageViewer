...
void ImGui::Textview(ImGuiID id,const char *text_Data,float allocation_pCentage,bool tool_tip,bool yaligin,bool xaligin)
{
	ImVec2 text_size=ImGui::CalcTextSize(text_Data,NULL);
	ImRect text_box(ImVec2(ImGui::GetCurrentWindow()->DC.CursorPos.x,ImGui::GetCurrentWindow()->DC.CursorPos.y),ImVec2(ImGui::GetCurrentWindow()->DC.CursorPos.x+(xaligin ==true?allocation_pCentage*ImGui::GetCurrentWindow()->Size.x : text_size.x) , ImGui::GetCurrentWindow()->DC.CursorPos.y + (yaligin == true ? 0.25*ImGui::GetCurrentWindow()->Size.y :( text_size.y+ImGui::GetFontSize()))));
	...
	ImGui::RenderTextClipped(text_box.Min,text_box.Max,text_Data,NULL,NULL);

	ImGui::GetCurrentWindow()->DrawList->AddRect(text_box.Min,text_box.Max,ImGui::GetColorU32(ImVec4(0.2,0.2,0.2,textregion_hover==true? 0.8:0.2)));

	ImGui::GetCurrentWindow()->DC.CursorPos.y+=(text_box.Max.y-text_box.Min.y) + ImGui::GetCurrentWindow()->WindowPadding.y;
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    ImVec2 pos = window->DC.CursorPos; //window position
    pos.y += window->DC.CurrentLineTextBaseOffset; //seek to next rendering coordinate
    ...
    float w_draw = ImMax(label_size.x, window->Pos.x + max_x);
